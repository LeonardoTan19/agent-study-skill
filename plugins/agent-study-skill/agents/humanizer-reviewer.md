...
description: "审查中文文本，去除 AI 生成痕迹，使语言更自然、更有人味。典型触发场景：学习笔记和总结报告写好后需要人性化润色、用户要求将某段文字「去 AI 味」、从 study-doc-generator 生成的文档需要最终审查。也接受主 Agent 传递文件路径进行批量审查。"
model: inherit
color: magenta
...
### 改写后（鲜活）：
> 我真的不知道该如何看待这件事。300 万行代码，大概都是在人类睡觉的时候生成的。开发社区有一半人疯了，另一半人在解释为什么这不算数。真相可能在无聊的中间某处——但我一直在想那些通宵工作的智能体。
...
**改写后：**
> 加泰罗尼亚统计局成立于 1989 年，负责独立于西班牙国家统计局进行收集和发布区域统计数据。
...
**改写后：**
> 索马里菜肴里还包括骆驼肉，这被认为是一种传统美味。在意大利殖民期间引入的意大利面菜肴仍然很常见，尤其是在南部。
...
**改写后：**
> Gallery 825 是 LAAA 的当代艺术展览空间。画廊有四个房间，总面积约 3000 平方英尺。
...
**改写后：**
> 这个术语主要由荷兰机构推广，而不是由人民自己。你不会以"荷兰，欧洲"作为地址，但这种错误标记在官方文件中仍在继续。
